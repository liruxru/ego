...
<script>
		...
		$("#chkAll,.eGoCheckbox").click(function() {
			...
			$(".chkAll").click(
					function() {
						//找到这个节点，查找里边的元素find
						$(this).parent().next().find(".check").prop("checked",
								$(this).prop("checked"));
						change();
						//计算小计在计算总价
						subTotal();
...
<img alt="" style="width: 100px;height: 97px;" src="${resources}/img/logo.png">
	...
	<c:if test="${ not empty storeCartList }">
	<!--        	描述：购物车主题条目       -->
	<div id="container">
		<!--标题和配送地址           -->
		...
		<!--
			...
			<c:forEach   items="${storeCartList}"  var="storeCart" >
				<div class="store" >
					<div class="checkAll">
						<input class="chkAll" type="checkbox"> <span>${storeCart.storeName }</span><a href="${root }/user/store/showStore?storeId=${storeCart.storeId}">进入店铺</a>
					</div>
					<div class="storeCart" id="${storeCart.storeId }">
						<c:forEach items="${storeCart.cartWihtGoodsList }" var="cartWihtGoods" >
							<div class="cartItem" id="${cartWihtGoods.cart.id}">
									<div class="chck">
										<input  value="${cartWihtGoods.cart.id}" name="cartId" class="check" type="checkbox" />
									</div>
									<div class="td-goods">
										<div class="goodImg">
											<a href="${root}/member/goXiangqing?id=${cartWihtGoods.goods._id}">
											<img style="width: 100px;height: 100px;" src="${resources}upload/${cartWihtGoods.cart.coverimg }">
											</a>
										</div>
										<div class="goodDesc"><a href="${root }/member/goXiangqing?id=${cartWihtGoods.goods._id}">${cartWihtGoods.cart.goodname }</a> </div>
									</div>
									<div class="td-props">
									<p>
									<a href="${root}/member/goXiangqing?id=${cartWihtGoods.goods._id}">
										${cartWihtGoods.goods.description }
										</a>
									</p>
										
									</div>
									<div class="td-price">
										<span class="goodPrice">${cartWihtGoods.cart.price }</span>
									</div>
									<div class="td-quantity">
										...
										<div class="changeNumDiv">
											<input type="text" class="changeNum" value="${cartWihtGoods.cart.num }">
										</div>
										<div class="addDiv">
			...
			<div id="cart-floatbar">
				...
				<div class="comm-right">
					...
					<a href="javascript:void(0)" class="submit_Btn">去结算</a>
